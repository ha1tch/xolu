# Decimal Types in xolu

This guide explains how to use fixed-point decimal fields in xolu for
...
xolu's decimal type stores values as exact fixed-point numbers with a
...
xolu will reject it with a type validation error.
...
## What xolu Does With Your Values

When you create or update an entity with decimal fields, xolu:
...
When you read the entity back, xolu divides by the scale factor and
...
xolu rejects decimal values in the following cases:
...
xolu must support fixed-point decimal values with:
...
- **format** is `"decimal"` — a custom format recognised by xolu
