...
## [0.30.23] XOT192 — pkg/timeseries: fixed a real, reproducible check-then-scan race across QueryRange, Latest, and Aggregate -- confirmed clean on real multi-core hardware (2000/2000 via cmd/critrepro's own ts-delete-fence scenario, plus a clean full whole-tree suite run) after being found via a flaky test initially misjudged as sandbox noise and reproduced precisely at 3/100 under -race on this project's own single-core sandbox (v0.30.23, 2026-08-13)

Theme: storage · closed 0.30.23 · 2026-08-13


- **Trigger:** a flaky failure of `TestDeleteTimeline_DeletingMarker`'s own "concurrent reader" subtest surfaced during an unrelated release's whole-tree suite run. Initially assessed as a one-off (passed 3/3 on immediate retry) and nearly dismissed as sandbox noise -- correctly challenged: the fact it passed most of the time on a single-core sandbox says nothing about its own behaviour under true multi-core parallelism, which is exactly the class of timing issue this project already has a dedicated subsystem (`cmd/critrepro`) for weeding out.
- **Precise reproduction, not assumed:** `go test -race -count=100` on this same single-core sandbox reproduced the failure at **3/100**, a real, non-zero, stable-ish rate -- not a fluke. `-race` itself never fired at any point across every run; this is not a Go memory-model data race (no unsynchronised concurrent memory access), it is a genuine, real logic-level ordering gap.
- **Root cause, traced precisely by reading the actual code rather than reasoning abstractly:** `QueryRange`, `Latest`, and `Aggregate` (`pkg/timeseries/store.go`) all shared one shape -- call `s.reg.get(timeline)` once, at the very top, to validate the timeline is defined and fetch its config, then proceed to build scan bounds and read from Pebble *without ever re-checking the deleting marker*. `DeleteTimeline`'s own teardown (`markDeleting`, then `getForDelete`, `ListRollups`, a `DeleteRollup` call per rollup, then `deleteDataRange` actually clearing the underlying data, then finally `reg.delete`) is comparatively wide -- real work happens across several steps. The reader's own gap between its `get()` check and its own scan is comparatively narrow by contrast -- a few lines of dims/limit validation and key encoding. On a single core, hitting this requires the scheduler to preempt at exactly the wrong instruction boundary and let the entire wide delete sequence run to completion before resuming the reader -- rare, but not impossible, hence 3%. Under true multi-core parallelism, the wide delete window runs continuously on its own core with no need to share time with the reader at all, and the reader's own narrow window just needs ordinary wall-clock overlap with it -- structurally the same shape of amplification as `cmd/critrepro`'s own canonical `five_bal_legs` case (a narrow, multi-step-operation timing window whose hit rate scales with genuine parallel "attempts", not with `go test`'s own repeated-invocation behaviour).
- **Fix:** all three functions (`QueryRange`, `Latest`, `Aggregate`) now re-check `s.reg.get(timeline)` a second time, immediately before returning their own scan results, and return the standard "not defined" error instead of the scan's own (potentially stale) results if the timeline is no longer visible. This narrows the window from "the entire scan duration" down to the width of one extra registry lookup immediately adjacent to the return statement -- a real, substantial reduction, not a perfect atomic guarantee (true atomicity would require holding a lock across the entire scan, a much heavier change given Pebble iteration can involve real I/O).
- **Two of the three functions (`Latest`, `Aggregate`) had zero test coverage of this failure mode at all before this fix** -- found and fixed proactively, on the strength of sharing the identical shape as `QueryRange`, not because any test had ever caught them. New tests written for both, matching `TestDeleteTimeline_DeletingMarker`'s own established concurrent-reader pattern.
- **Verification:** `go test -race -count=500` clean for the original test after the fix (0/500, down from a demonstrated 3% baseline); `-count=200` clean for both new tests. Confirmed all three tests genuinely catch the bug, not just happen to pass: reverted all three fixes together, re-ran, watched genuine failures reappear in all three (10/200, 4/200, 3/200 respectively), restored, confirmed clean again. Full whole-tree suite clean (41 packages, 0 failures), lint clean.
- **`cmd/critrepro` scenario, built:** a first attempt mirrored `five_bal_legs`'s own established shape exactly (a fresh, real, disk-backed store per iteration) and hung reliably on real multi-core hardware within the first couple of iterations -- but the hang was inside a plain `Append` call during setup, entirely unrelated to the delete-fence race this scenario exists to test, and traced to Pebble's own commit-pipeline log writer stuck `[runnable]` but unscheduled, the signature of scheduling/I/O pressure from rapid real store open/close cycling, not a logic deadlock. A real, separate, worthwhile finding on its own -- this project has hit the same shape before (T-139, a genuine Pebble file-descriptor leak from a different cause) -- but conflating it with the race-fix question would have made any failure here ambiguous about which question it was even answering. Rebuilt with a deliberately different design: one store, opened once and never explicitly closed (a short-lived CLI process; the cleanest way to keep store-lifecycle overhead fully out of the experiment is to have no lifecycle beyond process exit), reused across every iteration, each iteration getting its own fresh, unique timeline ID inside that one shared store. `cmd/critrepro/ts_delete_fence.go`, registered as `ts-delete-fence`, races all three fixed functions (`QueryRange`, `Latest`, `Aggregate`) independently and concurrently against the same delete on every iteration. Already wired into `release.py`'s own `critrepro` step with no further changes needed (that step runs every registered scenario).
- **Real-hardware confirmation, the decisive step -- clean, closing this item:** `ts-delete-fence` run on Horacio's own real multi-core hardware at `-n 2000` -- **2000/2000 pass, 0 fail.** The full whole-tree suite on the same hardware also clean: 3619 system tests plus 31 cache-integration tests passing, 0 failures, 5645 subtests total. This is not proof the race can never occur (no finite-iteration test is), but it is real, meaningful evidence at a sample size an order of magnitude larger than the single-core baseline that first found it -- the amplification concern this whole investigation was built to answer did not materialise: the fix holds under genuine parallelism, not just on the single core where it was first found and fixed.

Cross-ref: CHANGELOG 0.30.23.

## [0.30.20] XOT191 — Documentation sync: MANUAL.md and docs/OQL_API.md still reported UNION and subqueries as unimplemented after both features were built and closed (T-04, XOT188) -- both updated with accurate feature lists, a new dedicated Subqueries section added to docs/OQL_API.md, and the feature-matrix table (which had never been updated for UNION at all, a gap in the earlier UNION docs update) corrected (v0.30.20, 2026-08-13)

Theme: documentation · closed 0.30.20 · 2026-08-13


- **Trigger:** direct instruction to sync the manual, OQL docs, and any other reference documentation that still reported UNION and subqueries as unimplemented, following T-04 (UNION, closed v0.30.15) and XOT188 (derived tables, closed v0.30.17).
- **Found genuinely stale, fixed:**
  - `MANUAL.md`'s own OQL "Supported Features"/"Not supported" lists -- UNION/INTERSECT/EXCEPT and subqueries added to supported; "subqueries" removed from the not-supported list. Two new worked examples added.
  - `docs/OQL_API.md`'s own feature-matrix table (`## Limitations`) never mentioned UNION/INTERSECT/EXCEPT at all, even after the UNION prose section was updated at v0.30.15 -- a gap in that earlier update, caught now. `Subqueries` row corrected from "Not supported" to "Supported"; a new, distinct row added for "JOIN with a subquery on either side" (still genuinely, correctly unsupported -- a different claim than "subqueries" generally).
  - Three separate JOIN-section mentions of "subqueries are rejected" reworded for precision (they were correct in their own narrow context -- a subquery still cannot appear as either side of a JOIN, that combination was never built -- but the phrasing could be misread as a blanket statement about subqueries everywhere in OQL) and cross-referenced to a new section.
  - A new `## Subqueries (Derived Tables)` section added to `docs/OQL_API.md`, matching the depth and structure of the existing `## JOIN Queries` section: syntax, three examples, and every real constraint (alias required, column-alias-list unsupported, 10-level nesting cap, not on either side of a JOIN, no SQL-level push-down, the decimal/floating-point tradeoff for an outer SUM/AVG).
- **Checked and correctly left unchanged, confirmed not stale:** `docs/OQL_JOIN_PUSHDOWN.md` (a dated, "Status: Design" historical implementation spec specifically for JOIN, predating this session -- its own claims about subqueries on a JOIN side are still accurate); `docs/KNOWN_ISSUES.md`'s three "subquery" mentions (internal SQL subqueries used inside `bal`/`obj`'s own storage-layer transaction logic, unrelated to OQL's own query language entirely); `docs/SULPHER_QUERY_REFERENCE.md`'s own UNION section (Sulpher's own separate, already-correct, unrelated implementation); `README.md`, `docs/API_REFERENCE.md`, `docs/API_V2.md` (no relevant mentions at all); a handful of unrelated English-word/type-theory false positives in `docs/FSM.md` and `docs/GRAPH_API.md` ("a union of integer intervals", "intersect the results in the application layer").
- **Verification:** every new worked example added to `MANUAL.md` and `docs/OQL_API.md` (the UNION example, the subquery-filtering-on-an-aggregate example, nested subqueries, UNION nested inside a subquery) run directly against a real server and confirmed to actually execute successfully -- not just written to look plausible.

Cross-ref: CHANGELOG 0.30.20.

## [0.30.19] XOT190 — Follow-up to XOT186: two adversarial tests in the UNION and derived-table suites had been deliberately weakened while the aggregate bug was open, and several real combinations (GROUP BY as a UNION branch, HAVING inside either feature, a derived table's own outer query filtering on the inner aggregate, decimal SUM inside a derived table) had never been verified at all -- all now covered with genuine correctness assertions (v0.30.19, 2026-08-13)

Theme: query-engines · closed 0.30.19 · 2026-08-13


- **Trigger:** direct instruction to revisit the UNION (T-04) and derived-table (XOT188) work and check specifically for tests weakened or functionality left unverified because of XOT186 (the aggregate/GROUP BY correctness bug), which was open and broken for the entire time both features were built and tested.
- **What was found:** two adversarial subtests, one in each suite, had been deliberately written with weakened assertions at the time -- `TestOQLUnion_Adversarial`'s own "aggregate function within a UNION branch" and `TestOQLSubquery_Adversarial`'s own "aggregate inside the inner subquery" -- both could only honestly prove "the new machinery doesn't corrupt an already-broken aggregate path any further," not that aggregates actually worked in combination with UNION or derived tables at all. Additionally, several genuinely untested combinations were identified: `GROUP BY` as a UNION branch's own SELECT (only bare `COUNT(*)` had been tried); `HAVING` inside either a UNION branch or a derived table's own inner subquery at all (HAVING's own separate `exprAliases` bug, fixed alongside XOT186's main fix, had never been exercised through either feature); a derived table's own outer query genuinely filtering on the inner subquery's own aggregate result (the original intent of the derived-table test, abandoned at the time since it could not have passed, now restored); and decimal-typed `SUM` inside a derived table's own inner subquery.
- **Resolution:** both weakened tests replaced with real-correctness assertions -- verified genuine values, not just row-count consistency with a known-broken baseline. Five new subtests added across both suites covering the previously-untested combinations listed above, all passing against the now-fixed aggregate path. One of the new tests initially failed on a wrong assumption of its own (expected a numeric total, decimals correctly cross the wire as strings per this session's own established convention) -- caught and fixed before it shipped, not a product bug.
- **Deliberately not added, considered and deprioritised:** tenant isolation specifically for an aggregate query combined with UNION or a derived table. Tenant isolation for aggregate queries alone is already verified (`TestRegression_AggregateTenantIsolation`), and tenant isolation for UNION/derived tables alone is already verified separately -- the combination introduces no new code path at all, since each UNION branch or derived table's own inner query recurses through the identical `executeSelect` with the same tenant parameter already proven correct on its own. Genuinely low risk; noted rather than built, to avoid low-value test proliferation.
- **Verification:** full whole-tree suite clean throughout (41 packages, 0 failures), lint clean.

Cross-ref: CHANGELOG 0.30.19.

## [0.30.18] XOT186 — COUNT(*)/GROUP BY/aggregate correctness: fixed. Root cause traced by direct unit testing (Aggregator.Aggregate itself was already correct in isolation) then instrumenting the planner's own dispatch: GenerateAdaptedSQL and GenerateAggregateSQL both unconditionally emitted a WHERE tenant_id = ? predicate for adapted tables, which never have that column at all -- the generated SQL failed outright, and a second, independent bug (a failed push-down left plan.Push unchanged, incorrectly gating off the Go-path aggregate fallback that would otherwise have caught this) made the failure silent rather than loud. A third, related bug found and fixed alongside these: HAVING against the SQL push-down path's own result rows failed to resolve an aggregate expression like SUM(val) against its own declared alias (v0.30.18, 2026-08-13)

Theme: query-engines · closed 0.30.18 · 2026-08-13


- **Trigger:** found while writing an adversarial test for `COUNT(*)` inside a UNION branch (T-04's own follow-up work) -- the test failed with `cnt:<nil>` instead of a real count, and the union machinery itself was the first suspect. Isolated directly: the identical query with no UNION at all (`SELECT COUNT(*) AS cnt FROM some_entity`) has the exact same broken behavior. Not a UNION bug at all; UNION's own combine step is innocent here, simply passing through whatever the (already broken) aggregate path produces.
- **Symptom, confirmed directly against a real server, not inferred:** `SELECT COUNT(*) FROM entity` (with or without an alias, with or without `GROUP BY`) returns one row *per underlying record* with the aggregate column always `null`, instead of one aggregated row with the real count. Confirmed for three records: three rows returned, each `{"cnt": null}` or `{"COUNT(*)": null}`, `rows_returned: 3` in the stats.
- **Confirmed not blob-specific:** the identical broken behavior reproduces for a schema-registered (adapted) entity too, not just a plain blob one -- this isn't the blob/adapted dual-path distinction that's been the root cause of several other bugs closed this session (XOT173, XM-7b).
- **Update (2026-08-13, earlier), scope confirmed broader than originally filed:** found again while building and testing derived tables (XOT188), through a completely different query shape -- `SELECT cat, AVG(val) AS avgval FROM entity GROUP BY cat`, no `COUNT(*)`, no UNION, no derived table involved at this layer at all. Confirmed not specific to `COUNT(*)`, nor to any particular aggregate function.
- **Root cause, traced by TDD-style direct unit testing before touching any of the real code:** `Aggregator.Aggregate` (the Go-path aggregation function) called directly with synthetic records and a real parsed AST produced a perfectly correct result on the first attempt -- immediately, decisively ruling out the aggregator's own logic. Moving one level up and instrumenting the planner's own dispatch (temporary debug output showing the actual generated SQL, since the bug at this point turned out to be an *interaction* between components rather than any single function's own logic, where instrumentation was faster than further unit tests) showed the query planner always chooses `PushAggregate`/`PushFull` for any adapted entity with `GROUP BY` or an aggregate, and the generated SQL for both was `... WHERE tenant_id = ? GROUP BY ...` -- adapted tables are isolated by their own per-tenant table name (`t<XXXX>_ndata_<entity>`, confirmed directly against `SQLiteStorageDialect.CreateTableSQL`'s own comment, "no tenant_id column needed") and never have a `tenant_id` column at all. The query failed outright (`no such column: tenant_id`). A second, independent, compounding bug meant this failure was silent rather than loud or correctly recovered: `plan.Push` was never reset after a push-down failure, so the Go-path aggregate fallback (step 3 of `executeSelect`'s own pipeline) stayed incorrectly gated off by `!plan.pushed(PushAggregate)`/`!plan.pushed(PushFull)` even though the push-down had actually failed and fallen through.
- **Fix, two parts:** (1) removed the incorrect `tenant_id` predicate entirely from both `GenerateAdaptedSQL` (`pkg/oql/sqlgen_adapted.go`) and `GenerateAggregateSQL` (`pkg/oql/sqlgen_aggregate.go`) -- the same class of mistake as XOT173's own JOIN generator bug, just in two different files. (2) `executor.go`'s own `PushFull`/`PushAggregate` cases now reset `plan.Push = []PushDecision{PushNone}` on any generation or execution failure, so every downstream step correctly treats the query as unpushed and re-applies in Go -- verified as a genuine, independent safety net by reintroducing just the `tenant_id` bug with this reset still in place: the query still returned the correct result via the Go-path fallback.
- **A third, related bug found and fixed while writing the HAVING regression test:** `filterRecordsByExpr` (HAVING evaluation against the SQL push-down path's own result rows) resolved an aggregate expression like `SUM(val)` by looking up the row's own raw expression string as a key, or failing that, any key containing the function name -- both fail when the SELECT list gives the aggregate a different alias (`AS total`), since the push-down path's own result rows only ever carry the declared alias as a key, never also the raw expression string (unlike the Go-path `Aggregate`'s own output, which happens to carry both, masking this specific gap there). The lookup silently returned `nil`, and `havingCompare(nil, 50)` was false for every row -- a HAVING clause that looked like it ran (200, a result came back) but discarded everything regardless of whether it should have passed. Fixed with a new `Aggregator.SetExprAliases`, mapping each aggregate column's own stringified expression to its declared alias, consulted as an exact-match step before the existing "contains" heuristic.
- **Verification:** unit-level (`TestAggregator_HavingExprAliases` and two more, `pkg/oql`, constructing rows in exactly the SQL push-down path's own alias-only shape rather than the Go-path's own, to avoid masking the bug) and real end-to-end (`TestRegression_AggregateCorrectness`, `TestRegression_AggregateTenantIsolation`, `pkg/server`: `COUNT(*)`, `GROUP BY` with `AVG`/`SUM`/`MIN`/`MAX`, `HAVING`, the empty-entity SQL-standard single-row case, and tenant isolation specifically re-verified since a `WHERE tenant_id = ?` predicate was removed -- proven safe by the per-tenant table name mechanism, not assumed). Every fix confirmed to genuinely catch its own bug via revert-and-confirm. Full whole-tree suite clean throughout (41 packages, 0 failures), lint clean.

Cross-ref: CHANGELOG 0.30.18.

## [0.30.17] XOT188 — OQL derived-table support (subqueries in FROM: SELECT ... FROM (SELECT ...) AS alias) implemented, Tier-1 Go-side recursive execution matching the same architectural choice already made for UNION -- 17 subtests plus 12 validator-level cases, all passing (v0.30.17, 2026-08-13)

Theme: query-engines · closed 0.30.17 · 2026-08-13


- **Trigger:** direct instruction to implement and intensively test OQL derived-table support (subqueries in FROM: `SELECT ... FROM (SELECT ...) AS alias`), following the same cost-estimation and Tier-1 architectural analysis already done for UNION (T-04) earlier this session.
- **Prior state:** the parser already fully modelled this (`ast.DerivedTable`), but the validator explicitly, deliberately rejected it (`default: return "invalid table reference"`), including on either side of a JOIN. No execution support existed at all.
- **Implementation, Tier-1 (Go-side recursive execution), matching the same architectural choice already made for UNION:** `validateDerivedTable` (`pkg/oql/validator.go`) requires an explicit alias and rejects the positional column-alias-list syntax (`AS x(col1, col2)`) outright -- the inner result's own rows are Go maps with no defined key order, so honouring a positional rename would need tracking the inner SELECT list's own column order separately, real additional complexity for a rarely-used feature. `executeDerivedTable` (`pkg/oql/executor.go`) runs the inner subquery through the exact same `executeSelect` path every other query uses via plain recursion -- full tenant scoping, decimal denormalisation, adapted/blob resolution all inherited for free -- then applies the outer query's own WHERE/GROUP BY/ORDER BY/DISTINCT/TOP entirely in Go, reusing the existing, already-generic Go-path toolkit (`filterRecords`, `projectColumns`, `OrderBy`, `ApplyTop`, `distinctRecords`, `aggregator.Aggregate`) completely unmodified. Column resolution against the inner result's own plain keys already worked via `getFieldValue`'s own existing qualifier-stripping fallback, confirmed directly rather than assumed.
- **A new safeguard, since this is the first genuinely self-nesting OQL construct:** `derivedTableDepth`, a nesting-depth cap (10 levels), checked once upfront at the top-level `Validate` entry point via a plain AST walk -- deliberately not threaded through every recursive `validateSelect` call signature, and deliberately not mutable per-`Validator` state, to avoid any concurrency hazard if a `Validator` instance is ever shared across requests.
- **A known, honest, documented limitation:** no SQL-level push-down for a derived table's own outer query at all -- a real efficiency gap for a derived table over a large inner result, not solved here, matching the identical tradeoff already accepted for UNION. An outer `SUM`/`AVG` over a derived table's own decimal column uses the generic string-parsing numeric path rather than the raw-scaled-integer decimal-precise path (no live `AdaptedRegistry` entry exists for a synthetic, computed result set) -- a real, small floating-point-precision gap for that specific combination, not a correctness gap for anything else.
- **Verification:** 17 subtests across three test functions (`TestOQLSubquery_EndToEnd`, `TestOQLSubquery_TenantIsolation`, `TestOQLSubquery_Adversarial`, `pkg/server`) plus 12 validator-level cases (`TestValidatorDerivedTable`, `TestValidatorDerivedTableDepthCap`, `pkg/oql`) covering basic passthrough, column projection, inner+outer WHERE combined, GROUP BY/aggregate, ORDER BY, DISTINCT, TOP, two-level nesting, UNION nested inside a derived table, empty-result cases (either side), tenant isolation, MaxRows enforcement on the outer result, the depth-cap boundary tested precisely (exactly 10 levels passes, 11 fails), decimal correctness, nonexistent-entity rejection at validation (not a 500 at execution), and an honest test proving the derived-table machinery doesn't compound XOT186's own separate, unrelated aggregate bug. All pass. Full whole-tree suite clean, lint clean.
- **Two real, severe, pre-existing bugs found and fixed while building this, filed separately:** XOT189 (WHERE silently dropped for small adapted entities, unrelated to derived tables specifically); XOT186 updated with new evidence that its own scope is broader than originally filed (`GROUP BY` generally, not just bare `COUNT(*)`), found again through a derived table's own inner subquery.

Cross-ref: CHANGELOG 0.30.17.

## [0.30.17] XOT189 — Critical: WHERE was silently dropped entirely for any adapted entity small enough that the planner chose not to push the query down to SQL -- SQLiteStore.ListWithFieldsAndFilter's own adapted-table branch discarded its own predicate argument completely, returning every row unfiltered regardless of the WHERE clause, on any field including the primary key (v0.30.17, 2026-08-13)

Theme: storage · closed 0.30.17 · 2026-08-13


- **Trigger:** found 2026-08-13 while writing an edge-case test for derived tables ("empty inner result after a WHERE filter") -- the test returned the wrong row entirely instead of an empty result, and derived tables were the first suspect. Isolated directly: the identical query with no derived table at all had the same broken behaviour, and further isolation showed WHERE was broken on every field tested, including the primary key, for a plain, freshly-created, schema-registered entity -- not a derived-table bug at all.
- **Symptom, confirmed directly against a real server, not inferred:** `SELECT name FROM entity WHERE id = 999` (no match anywhere) returned every row in the entity, unfiltered. Reproduced with string equality, numeric equality, numeric range, and inequality on both the primary key and an ordinary field -- every condition type, every field, on any adapted entity small enough that the query planner chose not to push the query down to SQL (`plan.Push == none`, the "B4 path": `storage.FilterableStore.ListWithFieldsAndFilter`).
- **Root cause, confirmed precisely:** `SQLiteStore.ListWithFieldsAndFilter`'s own adapted-table branch (`pkg/storage/sqlite_field_query.go`) short-circuited to `adaptedList(...)` -- a plain, unfiltered listing function -- and returned its result directly, silently discarding its own `preds` argument entirely. The function's own comment ("adapted tables don't benefit -- their List already reads native columns") was simply wrong: `adaptedList` has no predicate-filtering capability of any kind. The caller (`executeSelect`'s own B4 path in `pkg/oql/executor.go`) trusted that predicates had already been applied and set `residualWhere` to only the *residual*, non-compilable remainder of the WHERE clause -- for a simple, fully-compilable predicate like `id = 1`, that residual is nil, so nothing was ever re-applied in Go either. A complete, silent WHERE bypass for the affected shape.
- **Severity:** critical. This affects any adapted entity small enough to stay under the push-down complexity threshold with a WHERE clause the predicate compiler can fully compile -- a very common, everyday query shape, not an edge case. Whole-tree suite passed throughout because no existing test happened to exercise this specific combination (small adapted entity, fully-compilable WHERE, `plan.Push == none`) before now.
- **Fix:** rather than trying to make `ListWithFieldsAndFilter` itself correctly apply predicates against an adapted table's own already-materialised rows (`jsonic.PredicateSet` has no capability to evaluate against a plain `map[string]interface{}` at all -- it's built specifically for token-stream scanning of blob JSON -- so doing this correctly there would mean genuinely new machinery), the B4 optimisation path is now skipped entirely for adapted entities (checked via the existing `storage.AggregateQueryable.IsAdaptedEntity`, the same interface the planner already uses for this exact purpose), falling through to the pre-existing, already-correct fallback: full list, full WHERE clause re-applied in Go.
- **Verification:** `TestRegression_AdaptedWhereFilter` (`pkg/server`) -- six real query shapes (WHERE id = match, WHERE id = no-match, WHERE string field = match, WHERE string field = no-match, WHERE numeric field = match, WHERE numeric field > range) against a real, minimal, freshly schema-registered entity, all correct. Confirmed to genuinely catch the bug: reverted the fix, watched the test fail exactly as originally found, restored it, confirmed passing. Full whole-tree suite clean afterward (41 packages, 0 failures), confirming the fix doesn't regress any existing behaviour despite touching a broadly-shared code path.

Cross-ref: CHANGELOG 0.30.17.

## [0.30.16] XOT187 — UNION adversarial testing found and fixed two real bugs: SELECT * silently bypassed the column-count validation entirely (collapses to a single AST entry regardless of real field count), and MaxRows was enforced per branch but never on the combined result -- a genuine resource-limit bypass via UNION (v0.30.16, 2026-08-13)

Theme: query-engines · closed 0.30.16 · 2026-08-13


- **Trigger:** adversarial testing of UNION/INTERSECT/EXCEPT (T-04's own follow-up work) -- deliberately probing where this session's own history says bugs hide (combinatorial interactions, resource limits, type coercion), not exhaustive enumeration.
- **Finding 1, fixed: `SELECT *` in a UNION chain bypassed the column-count validation entirely.** `SelectColumn.AllColumns` collapses `SELECT *` to a single AST entry regardless of the underlying entity's own real field count, so `validateUnionChain`'s own "same number of columns" check (`len(s.Columns)` on each side) saw `1 == 1` and passed for `SELECT * FROM wide UNION SELECT * FROM narrow` even when `wide` and `narrow` have entirely different real schemas -- confirmed directly: this produced a combined result with inconsistent row shapes (one row carrying an `age` field, the other not), not a clean rejection. Fixed by rejecting `SELECT *` outright in any UNION/INTERSECT/EXCEPT chain (`hasWildcardColumn`, checked on both the current link and every right-hand branch, since the last branch in a chain never has its own `.Union` set and would otherwise skip the check entirely -- a real bug in the first version of this fix, caught before shipping by tracing exactly which branch a two-way chain's own validation actually visits).
- **Finding 2, fixed: `MaxRows` was enforced per branch but never on the combined UNION result.** `executeSelect` already enforces the configured row cap independently for each branch (since each branch runs through it on its own), but `executeUnion`'s own combine step had no equivalent check on the assembled result -- N branches, each individually under the cap, could combine into something far over it. Confirmed directly: two branches of 3 rows each against a cap of 5 returned 200 with all 6 rows before this fix. A genuine way to bypass a configured resource safeguard using UNION specifically. Fixed by mirroring `executeSelect`'s own identical check against the final, combined result.
- **A third, real, but out-of-scope finding, filed separately as XOT186:** `COUNT(*)` was found broken independent of UNION entirely (returns one row per underlying record with the aggregate column always null) while writing an adversarial test for aggregates inside a UNION branch. Confirmed the identical standalone query (no UNION) has the same broken behaviour -- UNION's own machinery is innocent here, just passing through an already-broken aggregate path. Not fixed as part of this item; tracked separately given its unrelated root cause and its own severity.
- **Verification:** ten adversarial subtests added (`TestOQLUnion_Adversarial`, `pkg/server`) covering both fixes plus empty branches (either side, both sides), within-branch duplicate collapsing, chain-depth stress (10 branches), decimal denormalisation correctness per branch, NULL-value dedup consistency, and numeric type-coercion dedup consistency (int64 vs float64 producing the same row signature). Plus two new validator-level cases in `TestValidatorUnionChain` (`pkg/oql`) for the `SELECT *` rejection specifically, on both the left and right branch. Both fixes confirmed to genuinely catch their own bug: reverted each independently, watched the corresponding test fail with the exact original symptom, restored, confirmed passing again. Full whole-tree suite clean throughout (41 packages, 0 failures), lint clean.

Cross-ref: CHANGELOG 0.30.16.

## [0.30.15] T-04 — TD-002 — OQL executor support for UNION / INTERSECT / EXCEPT: implemented and verified end to end (2026-08-12) -- the executor-side implementation existed in the working tree with full validator-level test coverage, but no test had ever actually run a UNION query and checked the result; that verification gap is now closed, seven real end-to-end cases plus a dedicated tenant-isolation test, all confirmed to genuinely catch a real, deliberately-introduced failure before shipping (v0.30.15, 2026-08-12)

Theme: query-engines · closed 0.30.15 · 2026-08-12


- Parser AST already models these (`ast.SelectStatement.Union`, `ast.UnionClause.Type`), and did before this session -- not new.
- **Implementation status, found already substantially complete in the working tree, not built from scratch this session:** `validateUnionChain` (`pkg/oql/validator.go`) validates a chain's own homogeneity (every link must use the identical operator; mixed operators like `A UNION B INTERSECT C` are rejected outright rather than attempting SQL's own real precedence rules for mixed set operators) and column-count compatibility across every branch. `executeUnion` (`pkg/oql/executor.go`) runs each branch independently through the exact same `executeSelect` path every other query uses -- full tenant scoping, decimal denormalisation, adapted/blob resolution inherited for free, not reimplemented -- then combines results in Go via `combineUnionBranches`/`dedupeRows` (a deliberate choice over generating one nested SQL statement, at the cost of not pushing the combine step itself down to SQLite). `INTERSECT ALL`/`EXCEPT ALL` are rejected -- not valid T-SQL syntax at all, and correct multiset semantics for those two are real, separate, unimplemented work, not a formality.
- **The actual gap closed this session: verification.** `TestValidatorUnionChain` already existed and thoroughly covered validator-level rejection/acceptance, but no test anywhere had ever executed a real UNION query and checked its result -- the entire execution side was unverified. Closed with `TestOQLUnion_EndToEnd` (`pkg/server`), seven real cases against a live server (dedup, ALL, INTERSECT, EXCEPT, a three-way chain, trailing ORDER BY/TOP applying to the combined result rather than just the branch it's syntactically attached to, and column-count-mismatch rejection), plus `TestOQLUnion_TenantIsolation` proving tenant scoping genuinely holds rather than assuming it from the implementation's own comments. All eight found genuinely correct on first real execution -- confirmed to actually exercise the logic, not just pass vacuously, by temporarily disabling deduplication and watching the dedup-specific test fail with the exact wrong row count, then restoring it.
- **Docs corrected:** `docs/OQL_API.md` previously stated set operations were unsupported; now documents the real syntax, the two deliberate restrictions (homogeneous operator chains, matching column counts), and the ALL-variant rejection, with working examples.
- **Not done, explicitly out of scope for this pass:** SQL-level pushdown (the combine step runs in Go, not as one nested SQLite statement) -- a real, known efficiency gap for a large UNION over large tables, left as a documented limitation rather than solved here, matching the same tradeoff already made for XOT182/XM-5's own OFFSET/FETCH NEXT fix. Combining UNION with JOIN or a derived table in the same statement is untested and not claimed to work.

Cross-ref: CHANGELOG 0.30.15.

## [0.30.14] XOT184 — xoluman XM-7b: fixed -- generateJoinSelectColumns already called store.AdaptedColumnInfo (returning scale/isDecimal) for every column but discarded both entirely; now tracks decimal columns and reuses denormaliseAggregateDecimals (already shared by the adapted and aggregate paths) completely unmodified, since JOIN's own lack of aggregate-function support (XM-7a) means only that function's own "plain decimal column" default case ever fires (v0.30.14, 2026-08-12)

Theme: oql · closed 0.30.14 · 2026-08-12


- **Trigger:** xoluman team's own report (XM-7b), building a real "deals with their company's industry" saved query in the CRM example now that XM-3a (JOIN tenant scoping) is fixed.
- **Symptom, as reported:** `deals.amount`, a decimal field, correctly represented as a string everywhere else (`"333000.65"`), came back through a JOIN as an integer with the decimal point gone, scaled by exactly 100 (`33300065`) -- consistent across every row, `ORDER BY` still correct (a constant scale factor preserves relative order), confirming a display/serialization bug, not a computation error.
- **Root cause, confirmed precisely:** `generateJoinSelectColumns` (`pkg/oql/sqlgen_join.go`) already called `store.AdaptedColumnInfo` for every column -- the same call the adapted and aggregate paths use to identify decimal columns -- but discarded its `scale`/`isDecimal` return values entirely (`_, _, _, ok :=`). The JOIN generator knew which columns were decimal and threw that information away; the raw, scaled-integer stored value went straight to the client untransformed.
- **Fix:** `JoinSQL` gained a `DecimalColumns map[string]int` field; `generateJoinSelectColumns` now tracks decimal columns the same way `sqlgen_adapted.go`'s own `trackDecimalColumn` does; `executor.go`'s own JOIN case now calls `denormaliseAggregateDecimals` on the JOIN result -- the exact same function the adapted and aggregate paths already used, reused completely unmodified, since its own "plain decimal column" default case already covers JOIN's shape correctly (JOIN has no aggregate functions in its own SELECT list at all, XM-7a, a confirmed architectural limit, so no other case in that function's own switch ever fires for a JOIN result).
- **A second, real gap found and fixed along the way, same shape as the bug itself:** the `mockJoinStore` test double used throughout `sqlgen_join_test.go` always hardcoded `isDecimal = false` -- no unit test using it could ever have exercised decimal handling in the JOIN generator at all. Extended additively (a new, opt-in `decimalCols` field) to genuinely support this, confirmed backward-compatible with the full existing suite.
- **Verification:** a direct unit test (`TestGenerateJoinSQL_DecimalColumns`, `pkg/oql/sqlgen_join_test.go`) proving the SQL-generation-level tracking with three cases (adapted decimal column correctly tracked with its own scale; a blob-side column sharing the exact same field name as the adapted side's decimal column correctly left untracked; no decimal columns selected leaves `DecimalColumns` a non-nil empty map), and a real end-to-end HTTP test (`TestXM7b_DecimalThroughJoin`, `pkg/server`) reproducing xoluman's own exact value (`"333000.65"`) side by side through both a non-JOIN and a JOIN query. Both confirmed to genuinely catch the bug: reverted the fix at two separate points (the tracking logic itself, then separately the `executor.go` wiring), watched each test fail with xoluman's own exact reported symptom (the scaled integer, `33300065`, confirmed byte for byte via the end-to-end test), restored both, watched them pass. Full whole-tree suite clean afterward (41 packages, 0 failures), lint clean.
- **Re-confirmed against v0.30.10** by xoluman before this fix was written, not assumed stale.

Cross-ref: CHANGELOG 0.30.14.

## [0.30.13] XOT182 — xoluman XM-5: fixed -- root cause was a genuine no-op, not a rejection: stmt.Offset/stmt.Fetch parsed correctly into the AST but nothing in pkg/oql ever read either field; new ApplyOffsetFetch wired into the executor's own shared post-processing pipeline, which also, as a genuine bonus beyond xoluman's own reported scope, covers JOIN queries too since JOIN results already flow through the same pipeline (v0.30.13, 2026-08-12)

Theme: oql · closed 0.30.13 · 2026-08-12


- **Trigger:** xoluman team's own report (XM-5), verifying xolu's own response to XM-3b (the LIMIT resolution letter, which specifically recommended `OFFSET N ROWS FETCH NEXT M ROWS ONLY` as the native T-SQL pagination answer alongside `TOP N`).
- **Symptom, as reported:** `OFFSET N ROWS FETCH NEXT M ROWS ONLY` parsed and executed without error, but silently ignored both the offset and the row cap entirely -- returned every row, starting from the first.
- **This was on xolu's own side specifically, not just "a bug exists":** the response letter to XM-3b explicitly claimed this "already works, no extension needed," on the strength of a check that only confirmed the query returned `200` -- not that it returned the correct rows.
- **Root cause, confirmed directly:** a genuine no-op, distinct in shape from every other bug found this session -- `stmt.Offset`/`stmt.Fetch` were parsed correctly into the AST (confirmed directly, `tsqlparser`'s own grammar handles both fields), but nothing anywhere in `pkg/oql` ever read either one. Not a rejection (unlike JOIN+aggregate, XOT183), not a silent misinterpretation -- the clause simply vanished after parsing.
- **Fix:** new `ApplyOffsetFetch` (`pkg/oql/aggregator.go`), matching `ApplyTop`'s own established shape, wired into `executor.go`'s own shared post-processing pipeline right after `TOP`'s own step, applied unconditionally since neither clause is pushed to SQL anywhere in this codebase (a real, known efficiency gap for a large offset against a large table -- the full result set is still fetched before slicing in Go -- left as a documented limitation rather than solved here; correctness first).
- **A genuine bonus, verified directly rather than merely claimed:** JOIN query results (`pkg/oql/sqlgen_join.go`'s own output) already flow through this same shared pipeline before returning -- confirmed by testing `JOIN ... OFFSET ... FETCH NEXT ...` directly, want 2 rows, got 2, not the un-fixed 5. Not part of xoluman's own reported scope, not separately implemented -- simply inherited by sharing the pipeline.
- **Verification:** a direct unit test (`TestApplyOffsetFetch`, `pkg/oql/aggregator_offset_fetch_test.go`) covering xoluman's own exact reproduction shape plus every boundary case (offset alone, fetch alone, offset past the end of the result set, both absent, fetch exceeding what's left), and a real end-to-end HTTP test (`TestXM5_OffsetFetchNext_Applied`, `pkg/server/xm5_offset_fetch_test.go`) reproducing their own scenario. Confirmed to genuinely catch the bug: reverted the `executor.go` wiring specifically (the new `ApplyOffsetFetch` function itself has nothing meaningful to revert to, being entirely new code), watched the end-to-end test reproduce xoluman's own exact symptom (all 5 rows, starting from the first), restored it, watched it pass. Full whole-tree suite clean afterward (41 packages, 0 failures), lint clean.
- **Re-confirmed against v0.30.10** by xoluman before this fix was written, not assumed stale.

Cross-ref: CHANGELOG 0.30.13.

## [0.30.12] XOT185 — xoluman XM-8: fixed -- new POST /cal/calendars route wraps the already-working Manager.CreateCalendar, closing the loop end to end (verified with real HTTP calendar creation, no test-only facade at all, followed by a real booking propose/confirm/list); a genuine, pre-existing wrong-error-code mapping (cal.ErrCalendarExists) surfaced and fixed along the way (v0.30.12, 2026-08-12)

Theme: cal · closed 0.30.12 · 2026-08-12


- **Trigger:** xoluman team's own report (XM-8), independently verifying XM-2's `CalListBookings` resolution against real data -- and, separately, this project's own direct failure to catch this: XOT180's own cal capability audit thread (v0.30.8) built `CalListCalendars` and `CalListBookingsForBearer` without ever checking the create side, despite this session's own integration test comments explicitly stating "calendar creation is deliberately not on the wire" -- treated as an accepted, known design choice rather than investigated as a possible gap.
- **Symptom, as reported:** every `cal` REST operation required an already-existing `calendarID` -- none of them, nor any other route, created one. `Manager.CreateCalendar` already existed and worked, but nothing in `pkg/server` wired it to an HTTP route.
- **Fix:** new `POST /cal/calendars` route, pairing naturally with the pre-existing `GET /cal/calendars` (XOT180) as a standard REST create+list pair, on both the server (`handleCalCreateCalendar`) and client (`CalCreateCalendar`). `calendar_id` required; `default_state`/`match_policy` default sensibly server-side when omitted, matching the storage layer's own established defaults.
- **A second, real bug found and fixed along the way, not previously known:** `classifyCalError`'s own error-mapping table mapped `cal.ErrCalendarExists` (a conflict) to `xoluerr.ErrCalCalendarNotFound` ("not found") -- semantically the wrong code, sitting dormant purely because no route existed that could ever trigger it. Added the correct code (`ErrCalCalendarExists`, `XOLU-CAL008`) and fixed the mapping.
- **Verification, genuinely closing the loop, not just adding a route:** a real end-to-end integration test (`TestIntegration_CalCreateCalendar_ClosesTheLoop`) uses no test-only facade whatsoever -- `CalManagerForTest()` does not appear in it at all -- every step, including calendar creation itself, goes through real HTTP: create a calendar, propose and confirm a real booking on it, confirm both `CalListCalendars` and `CalListBookings` see it. Plus a conflict test (creating the same `calendar_id` twice) and a tenant-isolation test (same `calendar_id` on two tenants, confirmed no false conflict across tenants and neither tenant's own list reflects the other's) -- included from the start, matching this session's own established discipline. Mock tests (3) plus these three integration tests, all pass. Full whole-tree suite clean (41 packages, 0 failures), lint clean, full integration suite clean (305 passing, up from 299).
- **Re-confirmed against v0.30.10** before this fix was written, not assumed stale: xoluman's own report confirmed the gap still present at that version.

Cross-ref: CHANGELOG 0.30.12.

## [0.30.11] XOT183 — xoluman XM-7a: aggregate functions (COUNT/AVG) unsupported in a JOIN's own SELECT list -- confirmed during XOT180's own JOIN generator audit to fail with a clear, explicit error, not silently; xoluman's own report agrees this is an acceptable architectural limit, not asking for a fix, filed only for the acknowledgment to be on record (v0.30.11, 2026-08-12)

Theme: oql · closed 0.30.11 · 2026-08-12


- **Trigger:** xoluman team's own report (XM-7a), re-enabling JOIN in the CRM example's own saved queries now that XM-3a (tenant-scoping bug) is fixed.
- **Symptom:** `COUNT`/`AVG`/other aggregate functions in a JOIN's own SELECT list fail with a clear, explicit error (`XOLU-QL004: unsupported expression type in JOIN SELECT: *ast.FunctionCall`). A plain, non-aggregate JOIN works correctly.
- **Not a new finding -- already confirmed and deliberately left as an architectural limit, not a bug slated for a fix.** Directly investigated during XOT180's own oql JOIN generator audit thread (v0.30.7): confirmed the failure is a clean, explicit rejection, not a silent misinterpretation or a dropped `GROUP BY` returning ungrouped raw rows. xoluman's own report agrees and is not asking for a fix here -- their own Sulpher-based rewrite (`MATCH ... RETURN c.industry, count(d), avg(d.amount)`) already covers the "average deal size by industry" case they needed.
- **No action required.** Filed only so the acknowledgment is on the record, matching xoluman's own report structure, which lists this as one of five items even though it's confirmed-working-as-intended rather than open work.
- **Closing immediately** with this as the full and only content -- there is nothing to fix.

Cross-ref: CHANGELOG 0.30.11.

## [0.30.11] XOT181 — xoluman XM-6: nested (non-REF) object fields fail to insert into an adapted table -- PartitionData's own write side never JSON-encoded array/object-typed column values, even though ReassembleData, its own documented inverse, already expected exactly that shape on the read side; a direct, confirmed consequence of XOT178 exercising named-tenant adaptation for the first time (v0.30.11, 2026-08-12)

Theme: storage · closed 0.30.11 · 2026-08-12


- **Trigger:** xoluman team's own second-round consolidated report (XM-6), surfaced while re-verifying earlier reports against v0.30.5 -- a direct, confirmed consequence of XOT178 shipping in the same release (schema adaptation actually applying to named tenants for the first time).
- **Symptom:** `companies.address`, a plain nested object field (`{street, city, state, country}`, no `format: ref`), worked in every prior xolu version -- since v0.30.5, creating a company with this field set failed outright with `XOLU-ST006`, server log showing `sql: converting argument $2 type: unsupported type map[string]interface {}, a map`. Cleanly isolated: the identical request with `address` omitted succeeds.
- **Root cause, confirmed directly, not inferred:** `PartitionData` (`pkg/storage/adapted.go`), the shared write-side function both `adaptedCreate` and `adaptedUpdate` call, never JSON-encoded array/object-typed column values before storage -- `columnValues[i] = data[col.JSONField]` assigned the raw Go value (a `map[string]interface{}` for a nested object) directly, then handed it straight to the SQL driver as a bind parameter, which correctly rejects it. `ReassembleData`, this function's own documented inverse, already expected and `json.Unmarshal`'d exactly this JSON-text shape on the read side -- a genuine, one-sided round-trip bug: the read side was built correctly, the write side was never updated to match. Confirmed exactly why this was dormant this long: blob storage's own plain-JSON writes never needed this encoding step at all, and before XOT178, every entity was silently blob for any real named-tenant deployment -- this write path had never actually been exercised for a named tenant until that fix shipped.
- **Fix:** `PartitionData` now JSON-encodes `array`-typed and non-REF `object`-typed column values before assignment, matching `ReassembleData`'s own expected shape exactly. Fixed once, in the function both `adaptedCreate` and `adaptedUpdate` share -- benefits both callers, including the DXP `entity.update` path XOT175 already fixed for merge correctness.
- **Verification:** a storage-layer test (`TestAdaptedCRUD_NestedObjectField`, `pkg/storage/adapted_crud_test.go`) proving the full create + update round-trip, plus a real end-to-end HTTP test (`TestXM6_NestedObjectField_AdaptedEntity`, `pkg/server/xm6_nested_object_test.go`) reproducing xoluman's own exact `companies`/`address` scenario against a real schema-adapted tenant. Both confirmed to genuinely catch the bug: reverted the fix, watched the storage-layer test fail with xoluman's own exact reported error text byte for byte, restored it, watched it pass. Full whole-tree suite clean afterward (41 packages, 0 failures), lint clean.

Cross-ref: CHANGELOG 0.30.11.

## [0.30.10] XOT174 — Duplicate filing of xoluman XM-3b -- the same report this item was originally filed for was independently investigated and resolved as XOT179; this item was never closed at the time, left stale with the original, now-superseded hypothesis (v0.30.10, 2026-08-12)

Theme: oql · closed 0.30.10 · 2026-08-12


- **What actually happened:** this item was filed early, on first reading xoluman's own consolidated report, carrying only the outside-in symptom and an unverified guess at the cause ("likely a tsqlparser statement-splitting bug specifically triggered by the JOIN+LIMIT combination"). The real investigation happened later in the same session, in a separate pass that didn't cross-reference this filing -- and closed as XOT179 instead, with the actual root cause (`LIMIT` was never valid OQL/T-SQL input syntax at all, reproducing identically with no `JOIN` present -- not a JOIN-specific interaction as this item's own heading assumed) and the actual resolution ("won't implement" -- `TOP N` was correctly supported the whole time; xoluman redirected to it plus REST/`pkg/client` pagination alternatives).
- **This item's own original hypothesis was wrong**, not just incomplete: it assumed a narrower, JOIN-specific bug that a deeper look showed didn't exist -- the real gap was general, unconditional on JOIN being present at all.
- **Closing this now as a bookkeeping correction**, not a second resolution -- the actual technical record lives entirely in XOT179's own closure. Found while answering a direct question about the full status of the xoluman-reported campaign; worth noting as a process lesson in its own right: closing an item under a *new* ID without first checking whether an earlier, related filing already existed left a stale, contradicted record sitting open for several releases.

Cross-ref: CHANGELOG 0.30.10.

## [0.30.9] XOT180 — systematic adversarial + regression testing pass across cal, oql JOIN generator, DXP participant adapters, and a general tenant-isolation coverage sweep -- all four threads complete, ten real gaps found and closed across five subsystems, following Horacio's own diagnosis that XM-2/XM-3a/XM-4 slipping past the existing test suite means the suite's own design checked the wrong things or not enough things (v0.30.9, 2026-08-12)

Theme: testing · closed 0.30.9 · 2026-08-12


- **Trigger:** Horacio's own direct observation, in response to xoluman's XM-2/XM-3a/XM-4 findings and this session's own `CalListBookings` work: if a test suite's design missed real gaps, the design itself checked the wrong things or not enough things -- not just "three bugs, now fixed." Confirmed directly, not theoretically, before this item was even filed: `CalListBookings` (built this same session) initially had no tenant-isolation test, despite the JOIN fix (XOT173) minutes earlier in the same session getting exactly that class of test. Same shape of risk (new query, `tenant_id` in its own WHERE clause), checked for one feature and not the other, in the same sitting. Fixed directly (`TestIntegration_CalListBookings_TenantIsolation`) rather than deferred, since it was cheap and directly parallel to existing work -- but it's the clearest possible illustration of the pattern this item exists to hunt down more broadly.
- **The actual failure mode, stated precisely:** not "insufficient test volume" but two distinct kinds of blind spot, neither caught by adding more tests of the same shape:
  1. **Missing capability, not missing test.** `pkg/cal`'s entire read side (`Occupancy`) answers "is this slot free/busy," never "what booking occupies it." No test could have caught this, because the method to test didn't exist -- nobody building the feature had a reason to ask the question xoluman's own occupancy grid needed answered.
  2. **Combinatorial blind spot.** XOT173 (JOIN + non-empty tenantID) and XOT175/XM-4 (DXP `entity.update` bypassing read-merge-write) were both real bugs in code that was individually well-tested -- tenant-scoped JOINs were tested, adapted tables were tested, DXP `entity.update` was tested. The *specific combination* never was.
- **Scope, four concrete threads, not "test more" in general:**
  1. **cal, done (2026-08-11):** two confirmed, real capability gaps, not hypothetical -- both found by systematically listing every read-capable method across `pkg/cal`'s storage layer and checking which ones were actually reachable via any HTTP route. `Calendars()` and `LiveBookings()` both already existed and were already tenant-scoped, but neither was ever called from `pkg/server` at all -- genuinely unreachable, not merely undocumented. Closed with `CalListCalendars` (`GET /cal/calendars`, exposes the existing `Calendars()` directly) and `CalListBookingsForBearer` (`GET /cal/bookings/by-bearer`, the exact cross-calendar query named in this item's own original filing -- `LiveBookings()` alone couldn't answer it since it has no bearer filter at all, so a new `BookingsForBearer` storage method was added). Both proven correct at every layer: storage-layer tests (including a boundary case for `BookingsForBearer` covering a non-live booking and a second bearer's own booking, both correctly excluded), mock tests, real end-to-end integration tests, and dedicated tenant-isolation tests for both -- included from the start this time, not added after the fact.
  2. **oql JOIN generator, done (2026-08-11):** checked every conditional branch in `sqlgen_join.go` for XOT173's exact shape (branches written to look distinct but never both exercised) -- `resolveJoinTableNames` and the select/ON column resolution logic all confirmed genuinely correct, each branch does something distinct, not a copy-paste duplicate. Then hunted specifically for combinatorial gaps (individually-tested features never tested together, XOT173's own actual failure mode): tenant scoping had only ever been tested with `INNER JOIN`, never `LEFT`/`RIGHT`/`FULL` -- added a test proving join type and tenant scoping are genuinely independent. Tenant scoping had never been tested combined with a user-supplied `WHERE` -- added a test with an `OR` specifically (the precedence case that would silently break if the generator's own parenthesization were ever lost); it held. Three-way joins and JOIN+aggregate (`COUNT`/`GROUP BY`) are both architecturally unsupported -- confirmed directly, not just read in the code, that both fail with a clear, explicit error (`XOLU-QL004`) rather than silently misinterpreting or dropping part of the query. New tests: `sqlgen_join_test.go`, `TestGenerateJoinSQL/LEFT_JOIN_with_tenant_scoping...` and `.../user_WHERE_with_OR_combines_correctly_with_tenant_scoping...`.
  3. **DXP participant adapters, done (2026-08-11):** checked all six other adapters (`bal`, `timeseries`, `cal`, `obj`, `fsm`, `loc`) against `entity.update`'s exact bug shape (DXP path calling a different, lower-level write function than the regular path, silently dropping a merge step). None share it, and not by luck: `bal`/`timeseries` have no partial-update concept at all (a transfer or an append is always a new record, nothing to merge); `cal`'s `CalTransitionParams` requires a full, valid `Span` on every call, architecturally a full resupply rather than a patch; `fsm`'s DXP path and its regular HTTP path call the *identical* underlying function (`fsmApplyTransitionInTx`), confirmed by tracing both call sites; `obj` and `loc` both use narrow, named-column SQL with no JSON-document-style "merge the rest" concept anywhere. The actual reason `entity.update` alone had this bug: `entity` is the only primitive with a generic, schema-flexible JSON document store where "partial patch of an arbitrary field subset" is a meaningful operation at all -- not six adapters that happened to dodge a bullet, one primitive with a fundamentally different write model.
  4. **Tenant-isolation test coverage audit, general, done (2026-08-12):** systematically listed every handler in `pkg/server` returning a list-shaped result, cross-referenced against every existing `TenantIsolation`-named test, and checked each uncovered candidate directly rather than trusting the SQL's own `WHERE tenant_id = ?` by inspection. Six real, confirmed gaps found and closed, each with a new test proving isolation directly, not inferred: `handleDxpDefList`/`handleDxpTxnList` (the existing `CrossTenantIsolation` test proved txn instantiation can't cross tenants, never touched the list endpoints themselves); `handleFSMMachineList`; `handleEventList`; `handleSeqList` and, separately, the generic `handleGenList` (two genuinely different handlers behind similar-looking routes -- `/gen/seq` is a static route to a dedicated handler, `/gen/{type}` is the generic one, confirmed directly against the route table after an initial test accidentally exercised the wrong one); and `handleBlobList`, which surfaced a stale, inaccurate code comment claiming isolation was "tested in pkg/server" for blob storage -- checked directly, no such test existed anywhere, now fixed. One real self-caught mistake along the way: an early version of the dxp list test compared raw numeric IDs across tenants and flagged a false positive, since each tenant has its own independent id sequence and `id=1` colliding across tenants is expected, not a leak -- fixed to compare by count, which is what would actually catch a real violation. `v2_obj_promote_handlers.go` checked and correctly ruled out (no list-style read endpoint exists there at all).
- **Status: all four threads complete.** Closing.

Cross-ref: CHANGELOG 0.30.9.

## [0.30.6] XOT172 — xoluman XM-2: all three read-API gaps closed -- ts got its own minimum slice (5 client methods against a previously-untouched route surface), CalListBookings and BalListAccounts each required new storage-layer queries and server routes, not just client wrappers, since neither capability existed anywhere in the stack before this (v0.30.6, 2026-08-11)

Theme: client · closed 0.30.6 · 2026-08-11


- **Trigger:** xoluman team's own consolidated report (XM-2), building three exploration UIs against real xolu data (bal ledger view, cal occupancy grid, ts rollup inspector) and finding genuinely different gaps per primitive, not one uniform one.
- **ts, done in full for the stated minimum slice.** `TSListTimelines`, `TSGetTimeline`, `TSQueryRange`, `TSRollupList`, `TSRollupGet` -- wire types checked directly against the real server handlers rather than trusting xoluman's own proposed signatures, which caught two real discrepancies (`TSTimeline.ID`/`TSEvent.Timeline` are `int64` on the wire, not `uint32`; `TSQueryRangeRequest.Dims` is mandatory server-side and wasn't in the original proposal at all). Mock tests plus a real end-to-end integration test (`TestIntegration_TSReadSlice`) seeding a full timeline/events/rollup chain and exercising all 5 methods against it.
- **cal, required new storage and server code, not just a client wrapper.** `CalOpenings`'s own `Occupancy` type is bitmap-based -- tracks busy/free, never which booking occupies a slot; no method anywhere in `pkg/cal` could answer "what's booked" before this. Added `BookingsInRange` to `SQLiteBookingSource` (a real SQL query with a proper overlap predicate, `start_utc < to AND end_utc > from`, not a Go-side filter), a new `GET /cal/bookings` handler, and the client method. Proven correct at three layers: a storage-layer unit test covering every boundary case (fully inside, before, after, straddling each edge, and a same-span booking on a different calendar to prove `calendar_id` scoping is real), a mock test suite, and a real end-to-end integration test -- plus, found and fixed directly off XOT180's own diagnosis: an initial gap where this new query had no tenant-isolation test at all, despite the JOIN fix minutes earlier in the same session getting exactly that class of test. Fixed with `TestIntegration_CalListBookings_TenantIsolation`, now present.
- **bal, same shape as cal -- also required a new storage-layer query and server route.** No method anywhere in `pkg/bal` could enumerate a tenant's own accounts; every existing method requires an already-known `account_id`. Added `ListAccounts` to `bal.Store` (joins `bal_accounts` with `bal_balances` in one query, sparing callers an N+1 round trip), a new `GET /bal/accounts` handler, and the client method (`Minor int64` alongside the decimal-string `Value`, matching `BalBalanceResult`'s own established shape). Tenant isolation here works by a structurally different mechanism than cal's (per-tenant table-name prefixing via `tenant.TenantID.TablePrefix()`, not a shared table plus a `tenant_id` column) -- proven directly anyway, not assumed safer by construction: a storage-layer unit test (`TestListAccounts_TenantIsolation`, two `*bal.Store` instances over the same DB, different tenant IDs) and a real end-to-end integration test through the actual HTTP + tenant-resolution path (`TestIntegration_BalListAccounts_TenantIsolation`), included from the start this time rather than added after the fact.
- **bal's own open, non-blocking question, not addressed:** whether repeated `BalAsOf` at each point on a time axis is the intended pattern for a balance-over-time chart, or whether a range-native `BalHistory(accountID, from, to, step)` would be preferred. Left for xoluman's own follow-up, not assumed either way.
- **cal's own secondary, unconfirmed observation, not addressed:** whether calendar provisioning being invisible from the client side (calendars appear implicit, created by first use) is a genuine gap or a misreading on xoluman's own part. Flagged in their original report, not independently investigated here.
- **Full whole-tree suite clean throughout** (41 packages, 0 failures), lint clean, full integration suite clean (288 passing, 0 failures) -- confirmed after every layer of this item's own work, not just once at the end.

Cross-ref: CHANGELOG 0.30.6.

## [0.30.5] XOT179 — xoluman XM-3b: LIMIT N will not be added to OQL/tsqlparser -- a fix was built, fully tested, and deliberately reverted after finding it breaks genuine T-SQL (LIMIT is not reserved there, e.g. a real column named 'limit'); TOP N was correctly supported the whole time and needed no fix, xoluman redirected to it plus REST/pkg/client pagination alternatives (v0.30.5, 2026-08-11)

Theme: oql · closed 0.30.5 · 2026-08-11


- **Trigger:** xoluman team's own consolidated report (XM-3b), found in the same investigation as XM-3a (XOT173, a distinct, separately-fixed bug): `SELECT ... JOIN ... LIMIT 5` failed with `XOLU-QL004: only single statements are supported`, while the identical query without `LIMIT 5` parsed correctly.
- **Root cause, confirmed by direct reproduction against the actual parser (`github.com/ha1tch/tsqlparser`), not inferred:** `LIMIT` was never valid OQL input syntax at all -- OQL's query language is parsed by `tsqlparser`, a T-SQL parser, and T-SQL's own native row-limiting syntax is `TOP N`, not `LIMIT N`. `LIMIT` lexed as a bare, unrecognised identifier, so `SELECT ... LIMIT N` silently parsed as THREE separate statements (the `SELECT`, then `LIMIT` and `N` each as their own `ExpressionStatement`) rather than a parse error -- which xolu's own `pkg/oql/oql.go` correctly rejected on seeing more than one statement. Confirmed directly, correcting xoluman's own "JOIN + LIMIT" framing: reproduces identically with no `JOIN` present at all (`SELECT a.name FROM deals AS a LIMIT 5` alone fails the same way) -- a general LIMIT-parsing gap, not a JOIN-specific interaction.
- **A fix was implemented, fully tested, and then deliberately reverted -- recorded here for the actual reasoning, not just the outcome.** `LIMIT N` was added to `tsqlparser` as a new, additive trailing clause (new `LIMIT` token, new `ast.SelectStatement.Limit` field, parser support mirroring the existing `OFFSET`/`FETCH` handling), fully tested (six new cases plus a dedicated `TOP`-regression check, full existing suite green: 500 -> 508 individual test results, confirmed via a precise before/after diff against the parent commit that nothing pre-existing was lost), committed locally.
- **Reverted before shipping, on reconsideration of the actual tradeoff (Horacio's own call, and the right one):** `LIMIT` is not a reserved word in genuine T-SQL -- confirmed directly, `SELECT limit FROM t` (a real table with a column literally named `limit`, e.g. a `credit_limit`-style field) broke under the new keyword, when it parsed correctly before. `tsqlparser` is a separate, general-purpose, publicly reusable repository, not scoped to xolu specifically -- absorbing non-T-SQL vocabulary because one downstream consumer wants it has no natural stopping point, and degrades the parser's own correctness for its stated purpose (faithfully parsing real T-SQL) for everyone else who might depend on it for that. The commit was never pushed upstream; the local checkout was hard-reset to the exact, unmodified parent commit -- confirmed via `git status` showing clean parity with `origin/main`, zero trace remaining.
- **Resolution: "won't implement," not a bug fix -- xoluman redirected to syntax that already works and never needed fixing.** `TOP N` was correctly supported the entire time and requires no change; verified directly end-to-end against a real server, including the exact JOIN shape from the original report, plus `OFFSET N ROWS FETCH NEXT M ROWS ONLY` for pagination (also pre-existing, native T-SQL, no extension needed). A genuine, separate finding surfaced during this verification, passed along as practical advice rather than chased as a bug: when both JOIN sides select a same-named column without an alias, the result keys collide and one value silently overwrites the other -- recommend `AS` aliases in that case.
- **Response delivered to xoluman**, covering `TOP`/`OFFSET`/`FETCH NEXT` syntax with working, verified examples, the REST API's own `page`/`per_page` pagination parameters, and `pkg/client`'s `ListParams.Limit` (maps to `per_page`) -- three independent ways to get the same result depending on whether the caller needs it as part of a larger OQL query or just wants a bounded list of one entity type.

Cross-ref: CHANGELOG 0.30.5.

## [0.30.4] XOT173 — xoluman XM-3a: OQL JOIN queries fail against real tenant-scoped instances -- generated JOIN SQL references a.tenant_id / b.tenant_id which doesn't exist on the aliased table (SQL logic error), despite the generator's own intent to tenant-scope both sides; reproduced for INNER and LEFT JOIN across multiple table pairs, re-confirmed against v0.30.0 (v0.30.4, 2026-08-11)

Theme: oql · closed 0.30.4 · 2026-08-11


- **Trigger:** xoluman team's own consolidated report (XM-3a), found while investigating why several CRM example saved queries using JOIN were failing -- discovered after correctly reproducing WORKING JOIN queries first (an earlier, wrong conclusion that OQL had no JOIN support at all was self-corrected before this report; real, tested JOIN infrastructure exists in `pkg/oql/sqlgen_join.go` plus five dedicated test files).
- **Symptom:** a plain, syntactically-correct JOIN query, matching the test suite's own working examples in shape, fails at execution against a real tenant-scoped instance: `SELECT a.name, b.name FROM deals AS a INNER JOIN companies AS b ON a.company = b.id` → `XOLU-QL004: JOIN query failed: aggregate query failed: SQL logic error: no such column: a.tenant_id (1)`. Reproduced identically for `LEFT JOIN` and for a different table pair (`contacts`/`activities`).
- **Root cause, as reported from the outside (xoluman does not have storage-layer visibility to confirm):** `sqlgen_join.go` does generate tenant-scoping conditions for both aliases (`fmt.Sprintf("%s.tenant_id = %s", js.LeftAlias, ...)`, same for the right alias) -- the intent to scope both sides by tenant is present in the generator. But at execution time SQLite reports no `tenant_id` column exists on the aliased table. Reads as a mismatch between what the JOIN SQL generator assumes the underlying entity table's columns look like and what they actually are -- xoluman's own hypothesis: the non-JOIN query path may scope by tenant a different way (a separate per-tenant table/schema, not a `tenant_id` column) that the JOIN generator wasn't written against. Flagged as the symptom precisely rather than a guessed fix.
- **Worth investigating carefully, not just patching the symptom:** this touches tenant-scoping code specifically. The reported failure mode is a hard error (query fails loudly), not a silent cross-tenant leak -- but the underlying mismatch this reveals (generator assumptions about the schema vs. actual tenant-scoping mechanism) is exactly the kind of thing worth understanding fully before fixing, given what it touches.
- **Re-confirmed against v0.30.0**, not assumed stale: identical error, byte for byte, on a freshly-built instance.
- **Related:** XM-3b (JOIN + LIMIT parsed as two statements) filed separately (XOT174) -- found in the same investigation, but a distinct bug with what looks like a distinct root cause (tsqlparser's own statement-splitting logic, not the SQL generator).
- **Root cause, actually confirmed by reading the generator directly -- xoluman's own hypothesis above was correct in spirit, precisely confirmed here:** `GenerateJoinSQL`'s own tenant-scoping block had a genuine copy-paste bug -- `if !plan.LeftAdapted { ... } else { ... }` with both branches emitting the IDENTICAL `"%s.tenant_id = ?"` predicate regardless of whether the entity was adapted. Adapted tables carry no `tenant_id` column at all: `resolveJoinTableNames` already resolves an adapted entity to its own per-tenant table name (`store.AdaptedTableName`, e.g. `t0001_deals`) -- the table itself is the tenant scoping, a `tenant_id` predicate on it is not just wrong, it's redundant. Blob rows genuinely need the explicit predicate, and the bug left that side correctly alone -- the bug was specifically that the adapted branch wrongly copied the blob branch's own predicate. Confirmed directly why this shipped despite dedicated JOIN test coverage: every existing test in `sqlgen_join_test.go` passed `tenantID=""` -- the entire tenant-scoping branch was never once exercised by any test in the file before this fix.
- **Fix:** `pkg/oql/sqlgen_join.go` -- the adapted branch now emits nothing; only the blob branch emits the `tenant_id` predicate.
- **Verification, three layers, all confirmed to genuinely catch the bug (reverted the fix, watched every one fail with the exact reported error message byte for byte, restored it, watched them pass):**
  1. Unit-level (`pkg/oql/sqlgen_join_test.go`, `TestRegression_JoinTenantScoping_AdaptedSideOmitsColumn`) -- three subtests directly asserting the generated SQL's own predicate shape (both adapted, mixed, both blob).
  2. End-to-end (`pkg/server/xm3a_join_tenant_test.go`, `TestXM3a_JoinQuery_TenantIsolation`) -- a real HTTP JOIN query against a real tenant-scoped server, deliberately going one step further than the original report asked for: genuine tenant ISOLATION proven directly (two tenants, same entity names, distinct data, confirmed neither tenant's own JOIN result contains the other's data), not merely "the query no longer errors." A fix that stopped referencing `tenant_id` without the per-tenant table name actually scoping correctly would make the query succeed while silently leaking data across tenants.
  3. Full whole-tree suite clean afterward (41 packages, 0 failures), lint clean.

Cross-ref: CHANGELOG 0.30.4.

## [0.30.3] XOT178 — schema registration (POST /schema/{entity}) only ever applied the resulting adapted table to the server's default (tenantID 0) store instance, never to any named tenant's own separate store -- confirmed directly that "default" auto-registers to tenant ID 1, meaning schema-defined entities were silently blob for every real named-tenant deployment; found while investigating XM-4/XOT175 (v0.30.3, 2026-08-11)

Theme: storage · closed 0.30.3 · 2026-08-11


- **Trigger:** found while investigating xoluman's own XM-4 report (XOT175) -- specifically, while verifying the "adapted entity" reproduction case, a direct test-only check (`IsAdaptedForTest`, added for this investigation) showed `deals` was NOT adapted on the tenant-scoped store actual requests use, despite a schema having been registered for it moments earlier.
- **Root cause, confirmed directly, not inferred:** `handleCreateSchema`'s own `RegisterAdaptedEntity` call only ever ran against `s.storage` -- the server's default, `tenantID == 0` store instance. But `storeForTenant` creates a SEPARATE `*storage.SQLiteStore` instance, with its own independent `AdaptedRegistry`, for any non-zero tenant -- even in shared-database mode, pointing at the identical underlying SQLite file. Confirmed with a direct check: a `"default"`-named tenant auto-registers to numeric tenant ID **1**, not 0. Schema registration and per-tenant store creation were two entirely independent code paths with no coordination between them at all -- meaning any entity with a schema was silently blob, never adapted, for every named-tenant deployment's own real data operations, regardless of whether or when a schema was defined for it.
- **Fix, two new helpers in `pkg/server/server.go`:**
  - `registerAdaptedEverywhere(ctx, entity, schema)` -- replaces `handleCreateSchema`'s single-store call. Registers on `s.storage` plus every currently-cached tenant store (`s.tenantStores.Range(...)`), so a schema registered after a tenant has already been active reaches that tenant's own store immediately.
  - `replayAdaptedSchemas(ctx, store)` -- called once, right after a new per-tenant store is created in `storeForTenant`'s own slow path, before that store is ever returned to a caller. Replays every currently-loaded schema (`s.validator.LoadedEntities()` + `GetSchema()`) onto the new store, so a tenant created after a schema was registered still gets it applied.
  - Together these close the gap from both directions: schemas registered before a tenant exists, and tenants created after a schema exists.
- **Test-only diagnostic added for verification, kept as permanent test infrastructure:** `IsAdaptedForTest(tenantID, entity)` on `*server.Server` -- checks the adapted registry on the actual store a given tenant's own requests use, not `s.storage`, which is what naive test code would check first and get a wrong answer from for any non-zero tenant. Used directly in XOT175's own regression test (`TestXM4_DXPPartialUpdate_AdaptedEntity_PreservesUntouchedFields`) to confirm the fix genuinely reaches the right store, not merely that it runs without error.
- **Verification:** full whole-tree short suite clean (41 packages, 0 failures) after the fix; lint clean; `critrepro` clean (100/100, sanity check given the stakes of touching server-level tenant-store wiring). XOT175's own two regression tests exercise this fix directly -- the adapted-entity variant would fail at the `IsAdaptedForTest` assertion alone if this fix regressed, independent of whether XOT175's own field-preservation fix also held.

Cross-ref: CHANGELOG 0.30.3.

## [0.30.3] XOT175 — xoluman XM-4: DXP `entity.update` bypassed read-merge-write entirely, calling saveInTx directly with the raw partial patch -- destroyed every field a transaction didn't explicitly touch, both blob and adapted entities, for both REST and Sulpher reads alike (root cause confirmed by direct reproduction, not the graph-index hypothesis originally filed here) (v0.30.3, 2026-08-11)

Theme: storage · closed 0.30.3 · 2026-08-11


- **Trigger:** xoluman team's own consolidated report (XM-4), found while confirming a Sulpher aggregation query (`avg(d.amount)`) against real CRM example data -- a Sulpher-based rewrite of one of the two XM-3 queries.
- **Symptom, as originally reported:** `avg(d.amount)` grouped by company industry returned `null` for two industries, despite `count(d)` for those same groups being correct. Traced to the individual deals: every one of the 5 deals in those two industries had gone through the CRM example's own `close_deal_won`/`mark_deal_lost` DXP transaction earlier in the same seed run. `d.amount` was `null` for all five, even though the DXP transaction that touched each one only ever updates `stage`, never `amount`. xoluman's own report characterized this as Sulpher-specific -- a direct REST fetch was reported as showing `amount` present and correct.
- **Root cause, confirmed by direct reproduction (two committed regression tests, `pkg/server/xm4_repro_test.go`), not inferred from reading code:** `EntityAdapter.Execute`'s own `EntityUpdateParams` case (`pkg/storage/entity_dxp_adapter.go`) called `saveInTx` directly with the raw PARTIAL patch data (`tp.Data`) -- entirely bypassing the read-merge-write that the regular PATCH endpoint (`patchInner`) correctly performs. For a blob entity, `saveInTx`'s own JSON write overwrote the whole stored document with just the patch. For an adapted entity, `adaptedUpdate`'s own generated `UPDATE` statement touches every column in the table spec regardless of what the patch actually contains, so every column the patch didn't mention got overwritten with a zero value. This is a genuine storage-layer bug, not a graph-read-path bug: reproduced directly, BOTH REST and Sulpher showed the missing field identically, since both read the same, already-corrupted stored row. This directly answers both of the original filing's own open questions: (1) yes, specific to DXP's own update path -- the regular PATCH endpoint was never affected, confirmed by reading `patchInner`'s own merge logic directly; (2) not specific to Sulpher's own snapshot at all -- the OQL `GROUP BY owner: null` sibling anomaly xoluman also saw is almost certainly the identical root cause (corrupted stored data, not a second, independent read-path bug), though not independently re-verified against OQL specifically as part of this fix.
- **A second, distinct, broader bug was found and filed separately while investigating this one** (XOT178): schema registration never propagated to any named tenant's own store instance, meaning the "adapted entity" reproduction case initially couldn't even be tested correctly until XOT178 was fixed first. Both fixes shipped together.
- **Fix:** `EntityAdapter.Execute` now reads the existing document (adapted via `adaptedGetInTx`, blob via the same raw `SELECT` `patchInner` uses) inside the same transaction the write itself uses, merges the patch over it exactly as `patchInner` does (skipping `id`/`_version`), and calls `saveInTx` with the merged result instead of the raw partial patch.
- **Verification:** two new regression tests, one per entity kind, asserting both the touched field (`stage`) and the untouched field (`amount`) via both REST and a live Sulpher query. Confirmed to genuinely catch the bug, not just that they compile: reverted the fix, watched both tests fail with the exact reported symptom (`amount: <nil>` in both REST and Sulpher), restored the fix, watched both pass. Full whole-tree short suite clean afterward (41 packages, 0 failures), lint clean, `critrepro` clean (100/100) as a sanity check given this touches a core DXP execute path shared with T-168/XOT171's own earlier concurrency fix.
- **Re-confirmed against v0.30.0** before any fix was written, not assumed stale: re-ran the full CRM seed fresh, queried the identical deal (id 6) used in xoluman's own original reproduction -- `stage` correctly reflected the DXP update, `amount` still read `null` through Sulpher, confirming the bug was still present in the exact shipped version before work began.

Cross-ref: CHANGELOG 0.30.3.

## [0.30.2] XOT177 — wave_progress.py crashed (IndexError) the moment an XOT-prefixed id appeared in a wave's own blocker set -- blockers_by_wave's sort key still assumed every id has a hyphen to split on, a spot XOT164's own dual-format fix missed; fixed by porting register.py's own _id_num helper (v0.30.2, 2026-08-09)

Theme: tooling · closed 0.30.2 · 2026-08-09


- **Trigger:** the v0.30.2 release's own `generate` step failed -- `wave_progress.py` crashed with `IndexError: list index out of range` at `blockers_by_wave`'s own sort key, the first time this specific function was exercised with an XOT-prefixed id in a wave's blocker set since the ID format changed (XOT164).
- **Root cause:** `_ID_ALT` (the id-matching regex) was correctly updated at XOT164 to match both `T-NNN` and `XOTNNN`, but `blockers_by_wave`'s own sort key -- `int(t.split("-")[1])` -- still assumed every id has a hyphen to split on. `"XOT176".split("-")` returns a single-element list (`XOT` and `-`-less prefix have no hyphen at all), so `[1]` raises. XOT164's own fix updated the places touched at the time; this function, evidently not exercised then, kept the old assumption.
- **Fix:** ported `register.py`'s own `_id_num()` helper into `wave_progress.py` verbatim (both files already duplicate `_ID_ALT` deliberately, matching the standalone-script convention every tool in `scripts/` follows -- no shared import), and switched the one remaining broken call site to use it.
- **Verification:** confirmed no other instance of the same pattern remains in the file (`split("-")`, `[2:]`) before considering this done, not just the one call site that happened to trigger. `wave_progress.py --check` runs clean afterward.

Cross-ref: CHANGELOG 0.30.2.

## [0.30.2] XOT176 — CI workflow review found go test was missing -short, silently running the full suite (stress tests included) on every push/PR despite the file calling itself the quick tier; fixed, and critrepro split into its own job for independent visibility and re-run, separate from the base test job (v0.30.2, 2026-08-09)

Theme: tooling · closed 0.30.2 · 2026-08-09


- **Trigger:** Horacio's own direct request to review what CI actually runs, given both `go test` and `critrepro` needed to genuinely be running there -- "take a look what we're actually running in CI."
- **Real gap found, confirmed by reading the workflow and the tests it gates, not assumed:** `ci.yml`'s own `go test ./... -count=1` step was missing `-short`. Confirmed directly that stress tests and "_Benchmark"-named tests both gate on `testing.Short()` (checked `pkg/storage/stress_test.go` and `pkg/oql`'s `TestFullPD_Benchmark`) -- without `-short`, CI silently ran the entire suite, every stress test included, on every single push and PR, despite the file's own header comment calling itself "quick tier, unit tests." Neither `release.yml` nor `cross-build.yml` run any tests at all, so this was the only place `go test` ran anywhere in CI -- there was no actual quick/full split in practice, just an unintentionally-full "quick" tier.
- **Fix:** `-short` added, restoring the actual quick-tier scope the header already claimed. Stress tests remain covered where this project's testing discipline already puts them -- release verification (`scripts/release.py`'s own test step) and local `make test --full` -- not every PR.
- **Separate change, same session:** `critrepro` split from a step inside the `test` job into its own job (`needs: test`, preserving the original "don't bother if the base suite already failed" ordering). Reasoning: it isn't a correctness check like `go test` -- it exists specifically because `go test`'s own `-count` harness turned out to be an unreliable instrument for a real class of bug (T-168/XOT171's own predecessor investigation). A distinct job gives it its own visible pass/fail in PR checks rather than being buried as the last step of a "test" job's log, and lets it be re-run on its own without re-running build+vet+c04dcheck+test. Confirmed `actions/setup-go@v5` caches the module download cache by default (keyed on go.sum's hash, shared across jobs and runs via GitHub's own Actions cache backend) before committing to the split, since a separate job's overhead was the real cost to weigh against the benefit.
- **Verification:** YAML validated for syntax; `go test ./... -short -count=1` run locally, confirmed stress tests genuinely skip; the exact same commands the workflow now runs were run directly in this sandbox rather than only inspected as text.

Cross-ref: CHANGELOG 0.30.2.

## [0.30.2] XOT171 — removed the first-run/previous-run progress disclaimer messages entirely per direct request; separately found and fixed a genuine performance regression the live-progress work introduced -- TestRebuildFrom_CorrectAndBounded (a real wall-clock performance-ceiling test) started failing on M1 because the streaming progress loop's own per-line JSON parsing competed for CPU with the test run it was observing (v0.30.2, 2026-08-07)

Theme: tooling · closed 0.30.2 · 2026-08-07


- **Trigger, part one:** Horacio's own direct request to remove the first-run/previous-run disclaimer messages entirely -- "the progress bar was great regardless." Done: both `(progress is an estimate from ...)` lines removed; the header now goes straight to the bar, whose own "~" prefix on the numbers already signals "estimate" without a full explanatory sentence.
- **Trigger, part two, the real finding -- a genuine regression, reported alongside the above:** `TestRebuildFrom_CorrectAndBounded` (pkg/cal) failed under `make test` on M1 immediately after XOT169/XOT170's own live-progress work landed, having never failed before it. Investigated directly rather than assumed unrelated or dismissed as a pre-existing flake.
- **Root cause, confirmed by reading the test itself, not guessed:** the test measures real wall-clock time (`time.Since(start)`) rebuilding a calendar index and asserts `perBooking > perBookingCeiling` -- a genuine performance-ceiling test, inherently sensitive to whatever else is consuming CPU while it runs. `run_unsharded_live`'s own streaming loop called `json.loads()` on EVERY line of `go test -json`'s output stream, including every "output" and "run" event never actually used for progress tracking -- real, ongoing CPU work in the parent Python process, running concurrently with the actual test binary on the same real multi-core hardware, competing directly for CPU time with the very performance-sensitive test it ended up perturbing.
- **Fix:** a cheap substring pre-check (`"Action":"pass"` / `"fail"` / `"skip"`, Go's own compact JSON marshaling confirmed directly against a real sample, no spaces after colons) skips full JSON parsing for the large majority of lines that structurally cannot be a top-level pass/fail/skip event -- json.loads() now only runs on lines that already contain one of these three exact substrings. `os.nice(10)` added alongside it, so what CPU work does remain yields scheduling priority to the actual test binary rather than contending with it on equal footing (wrapped in try/except -- harmless on platforms without it).
- **Verified what could be verified here, honest about what couldn't:** `make test` end-to-end after the fix, exit 0, all 3552 system tests passing including the previously-failing one under whatever load this sandbox's own execution produced. But this sandbox is single-core -- it cannot genuinely reproduce the multi-core CPU contention that caused the original M1 failure, the same limitation this session's own T-168 investigation ran into repeatedly. The fix is well-reasoned and directly addresses the confirmed mechanism (less CPU work competing for the same cores, lower scheduling priority for what remains), but a clean pass here does not by itself prove the M1-specific regression is gone -- that needs Horacio's own re-run to confirm.
- **Also found and worth a separate note, not fixed here:** while testing this, an interrupted/killed `go test` run left a truncated `test-output.json` behind, which the next run's own "previous run" estimator read as a genuine total (a spurious ~59 instead of the real ~3597) -- the estimator currently trusts any previous file that parses as valid JSON lines, with no check that the stream actually completed. A real `Ctrl-C` mid-run could produce the same thing for an actual user. Flagged for Horacio's own judgement on priority, not addressed as part of this fix.
- **M1 confirmation, the actual evidence closing this item:** Horacio ran `make test` twice on the real M1 hardware where the regression was originally observed. Both runs clean, `FAIL: 0`, `TestRebuildFrom_CorrectAndBounded` passing both times (absent from both failure lists, present in neither) -- 118s and 114s elapsed, in line with pre-regression timing. This is the verification record the sandbox-only pass above explicitly could not provide on its own.

Cross-ref: CHANGELOG 0.30.2.

## [0.30.1] XOT170 — first-run progress estimate no longer apologizes and shows a running count with no denominator -- a fast, previously-unmeasured static source scan (grep against *_test.go files, no Go toolchain invocation, 0.02-0.14s) now provides an honest ~estimate on the very first run too, not just subsequent ones (v0.30.1, 2026-08-07)

Theme: tooling · closed 0.30.1 · 2026-08-07


- **Trigger:** Horacio's own direct challenge to XOT169's first-run message ("no previous run found -- showing a running count instead"), calling it a lazy effort and asking whether a "more or less" accurate count could be found quickly enough.
- **The challenge was fair, and the gap was real:** XOT169's own investigation had only measured and ruled out the expensive, compile-based `go test -list` approach (76.7s across 46 packages). A plain source scan for top-level `Test` function declarations was never tried at all -- not considered, not measured, not ruled out on evidence. Once actually measured: 0.02-0.14s (effectively free, no Go toolchain invocation at all, just grep against `*_test.go` files directly), 3627 versus the real 3597 top-level tests (~0.8% high, since a plain grep doesn't respect build constraints the way `go test` does) -- close enough for a live, honestly-labelled estimate, and roughly 500-2000x faster than the compile-based approach that was the only one actually tested before.
- **Fix:** new `estimate_total_via_static_scan()` in `scripts/runtests.py`, used as the fallback specifically when no previous `test-output.json` exists (first run ever, or after a clean checkout) -- the previous run's own real count is still preferred whenever available, since it reflects an actual past execution respecting build tags rather than a naive source count. The first-run message changed from an apology to a description of what's actually happening: "progress is an estimate from a quick source scan (no previous run to go on yet)."
- **Verified genuinely, not just asserted:** the previous run's own `test-output.json` backed up, removed to force the first-run path, and the live bar captured under a real forced TTY (the standard `script` utility, since this sandbox's own stdout is never a real terminal) showing `~1237/3627` mid-run -- a real, working progress bar driven by the static-scan total, not the old no-denominator fallback. The previous-run path re-verified immediately after (backup restored) to confirm no regression -- bar correctly showed `~193/3597`, the real prior count, unchanged. `make test` run end-to-end afterward, clean, all existing summary fields intact.

Cross-ref: CHANGELOG 0.30.1.

## [0.28.3] XOT169 — make test now shows live, ANSI-redrawn progress (an honest ~estimate from the previous run's own test count, not a fabricated exact total -- an accurate upfront count was measured and found to cost 77s on its own, nearly doubling the run) and exact granularity previously missing from the summary -- subtest-inclusive totals and the full name of every skipped test, not just counts (v0.28.3, 2026-08-07)

Theme: tooling · closed 0.28.3 · 2026-08-07


- **Trigger:** Horacio's own request while waiting on a full test-suite run -- live progress with an ANSI-redrawn in-place status line, ideally an ASCII progress bar; and, separately, better numeric granularity than the summary previously gave -- exact subtest counts (not just top-level test counts) and the actual names of skipped tests, not just a bare count. Both explicitly framed as "in previous incarnations of the test harness we had better granularity," prompting a direct check of run_tests.sh's own header comment, which confirmed a real, ~300-line bash predecessor existed before T-139-adjacent consolidation into scripts/runtests.py -- not chased further once the current implementation's own data turned out to already carry everything needed for both asks.
- **Live progress -- accurate denominator checked directly, not assumed cheap:** an exact upfront count (summing `go test -list` across every package) was measured before deciding against it -- 76.7s for 3,587 top-level tests across 46 packages, against the ~116s the actual run takes; counting first would have nearly doubled `make test`'s own wall time just to know a percentage. Uses the previous run's own `test-output.json` instead -- free (already on disk), and test counts don't swing wildly run to run, so it's a genuinely useful estimate. Labelled "~" throughout so it reads as an estimate, not a promise; falls back to a running count with no bar or percentage when no previous run exists. Subtests are deliberately never part of this estimate or the live bar -- `t.Run()` subtests are generated at runtime and cannot be enumerated in advance by any method, not just the expensive one just ruled out, so a live "percent of subtests complete" would be fabricated, not merely approximate.
- **Mechanism:** `go test -json ./...` switched from a blocking `subprocess.run()` (write everything to a file, parse afterward) to a streamed `Popen`, read line by line -- every line still lands in the same on-disk json_path exactly as before, so `build_report()` and everything downstream of it is completely unaffected; the streaming read is additive, not a replacement for the existing file-based parse. Redrawn via `\r` + right-padding to clear stale characters, throttled to a minimum redraw interval so a fast run doesn't flood the terminal, though the underlying counts still update on every event regardless of redraw throttling. TTY-gated by reusing `colour_enabled()`'s own existing interactive-terminal check -- no second, possibly-divergent detection -- so a log file or piped output (CI, `tee`, this session's own sandbox) gets the original, unmodified blocking behaviour with zero ANSI sequences in the file, and only an actual terminal gets the live bar.
- **Granularity -- the underlying data was already being computed, just filtered out before reaching the summary.** `build_report()`'s own top-level-only filter (`"/" not in test`) was deliberate, matching `run_tests.sh`'s own prior counting convention -- extended, not replaced, to also track the subtest-inclusive totals and the full name of every skip (top-level or subtest) alongside the existing filtered counts. A new "SKIPPED" section (matching the existing "FAILURES" section's own style) lists every skip by full name; a new "Subtests run:" summary line shows the true, subtest-inclusive total.
- **Verification:** the exact numbers cross-checked against an independent source rather than trusted on their own -- the new "Subtests run: 5497" line matches `scripts/release.py`'s own "Tests run: 5497" reported throughout this same session, confirming the two tools agree on what a test count means despite computing it independently. The live-progress path verified two ways: a raw PTY capture (Python's own `pty` module, since this sandbox's own stdout is never a real terminal) showing the bar genuinely redraw via `\r` from 0% to 100% and land on the identical final summary and exit code as a plain, non-TTY run of the same suite; and the standard `script` utility confirming a clean `EXITCODE:0` under a real forced TTY. `make test` (the actual entry point) run end-to-end afterward, exit 0, all new fields present. `testrun.py`/`release.py` (which share none of the touched code) confirmed to still import and behave identically -- this work was scoped to `scripts/runtests.py` alone.

Cross-ref: CHANGELOG 0.28.3.

## [0.28.1] XOT168 — dxp/bal WriteTargets serialization fix (append_only accounts shared across concurrent transfer participants) landed correctly in both dispatchPhased and dispatchCollapsed, but go test -count=500 kept failing at ~33% on M1 even after the fix -- five hypotheses ruled out directly (Go race, broken lock, lost WriteTargets, driver comparison, clock non-monotonicity) before confirming the harness itself, not the application code, was the cause; new cmd/critrepro tool (wired into release.py and CI) runs critical scenarios outside go test's own repeated-invocation harness going forward (v0.28.1, 2026-08-07)

Theme: server · closed 0.28.1 · 2026-08-07


- **Trigger:** Horacio's own M1 report -- `TestDxpTxnAPI_Scale_FiveBalLegs_AllCommit` (five independent `bal` transfer participants sharing a source account, dispatched one goroutine per participant) failed once under plain `make test`, passed on immediate retry. "Didn't we see it before? Maybe it only happens the first time."
- **The real bug, confirmed and fixed:** an `append_only` account's own monotonic-timestamp guard (`XOLU-BAL006`) rejecting a perfectly ordinary concurrent write as "backdated." `bal.Adapter.Execute`'s own `executionInstant()` call is made independently per goroutine, with the resulting timestamp checked against the journal for that same account -- nothing kept timestamp-generation order consistent with actual commit order across participants sharing an account, until this fix. Closed with `dxp.Claim.WriteTargets` (both sides of a transfer, not just `Resource`'s own source-only value -- a real gap found directly, not assumed: `Resource` alone would have missed two participants sharing only a *destination*, a case a dedicated new test now covers) and a per-write-target `sync.Mutex` in **both** dispatch functions, `dispatchPhased` and `dispatchCollapsed`, held from immediately before `Execute` through `Commit`/`Abort`.
- **Two real wrong turns along the way, recorded honestly rather than smoothed over:**
  1. The first fix touched only `dispatchPhased`. Confirmed directly against a real server (not assumed from the def's own "pattern" field, which does not determine routing) that this exact scenario -- five `bal` legs, single tenant, all-SQL -- is always `CollapseEligible && EngineHomogeneous` and therefore always routes through `dispatchCollapsed`, a structurally different function (one shared `*sql.Tx` across all participants, not five independent ones) that the first fix never touched at all. The failure rate barely moved (~43%) because the fix was applied to a function this scenario never uses.
  2. After fixing `dispatchCollapsed` too, a debug trace claiming to "prove the lock works" was run on this project's own single-core sandbox -- where goroutines never truly run concurrently regardless of whether the lock exists. That trace was not real evidence, and was retracted directly once recognised, before it could be relied on further.
- **The harness finding, the actual reason this closed:** even with the correct fix in both functions, `go test -count=500` kept failing at a stable ~33-34% rate on real M1 hardware. Five hypotheses were checked directly and ruled out in turn, not guessed past: a Go-level data race (`-race` clean, 100/100 -- though this only rules out unsynchronized memory access, not a logical ordering issue, a distinction initially overstated as stronger evidence than it was); a broken lock (a sequence-numbered, wall-clock-timestamped trace showed clean, non-overlapping acquire/release pairs under genuine multi-core interleaving); lost `WriteTargets` (same trace confirmed correct population); a `modernc.org/sqlite` driver comparison bug (a direct, isolated probe of the exact `at > ?` pattern, two `time.Time` values a microsecond apart, compared correctly); and wall-clock non-monotonicity (a direct 2,000,000-iteration probe of `time.Now().UTC()` -- the exact call `xolutime.Now()` makes -- found zero backward jumps on the same M1, killing a theory that had explained every other observation reasonably well). A `payment0`-skew pattern in the failure log, initially read as evidence of something structurally special about that participant, turned out to be a reporting artifact (`dispatchCollapsed`'s own failure-scan loop always reports the lowest-indexed failure when multiple participants fail together) -- caught before being acted on, not after.

  Horacio's own hypothesis -- that `go test`'s own repeated-invocation harness, not the application code, was the actual source of the failures -- was the one that resolved it. A standalone reproducer (`cmd/critrepro`, built specifically to test this, mirroring the real test harness's own server construction exactly but with zero `testing.T`/`-count`/`go test` involvement) ran the identical scenario, same fix, same M1 hardware: 500/500, zero failures, twice over (once at n=500 directly, matching the original failing count precisely).
- **Not investigated further, deliberately:** "totally looks like a Go bug" in Horacio's own words, and this project does not have the budget to dig into the toolchain itself right now -- production traffic never goes through `go test` regardless, so the fix's own correctness (now confirmed, not merely argued for) is what actually matters. Filed as G-18 in `docs/KNOWN_ISSUES.md`'s own dormant-guards record, not chased into the Go runtime.
- **What ships:** the `WriteTargets` fix in both dispatch functions; a new test (`TestDxpTxnAPI_Scale_FiveBalLegs_SharedDestination_AllCommit`) covering the destination-sharing case the original bug report never exercised; `cmd/critrepro`, a permanent, extensible tool (not a one-off script) for running scenarios repeatedly outside `go test`'s own harness, wired into both `scripts/release.py` (200 iterations, a new `critrepro` step) and `.github/workflows/ci.yml` (100 iterations, every push and PR) going forward, for this scenario and any future one with the same true-multi-core-timing shape. `TestDxpTxnAPI_Scale_FiveBalLegs_AllCommit` itself keeps its own single-run coverage in the normal suite unchanged -- a real, valuable correctness check on its own -- with its doc comment updated to explain why repeated verification now lives elsewhere.

Cross-ref: CHANGELOG 0.28.1.
