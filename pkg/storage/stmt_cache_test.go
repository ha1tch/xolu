...
func TestStmtCacheBasic(t *testing.T) {
	...
	query := `SELECT COUNT(*) FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE tenant_id = ? AND entity_type = ?`
...
func TestStmtCacheMultipleQueries(t *testing.T) {
	...
	q1 := `SELECT data, _version FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE entity_type = ?`
	q2 := `SELECT COUNT(*) FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE tenant_id = ? AND entity_type = ?`
...
func TestStmtCacheConcurrency(t *testing.T) {
	...
	cache := store.stmtCache
	q := `SELECT COUNT(*) FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE entity_type = ?`
...
func TestStmtCacheQueryWithPlan(t *testing.T) {
	...
	sql := `SELECT data, _version FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE entity_type = ? AND json_extract(data, '$.type') = ?`
...
func TestStmtCacheAggregateQuery(t *testing.T) {
	...
	sql := `SELECT json_extract(data, '$.category') AS category, COUNT(*) AS cnt FROM ` + tenant.TenantID(0).NodesTableName() + ` WHERE entity_type = ? GROUP BY json_extract(data, '$.category')`
