...
## [0.24.4] - 2026-08-03 (tooling only, no `pkg/` source changed: two independent findings. `scripts/testrun.py`'s `SPLIT_THRESHOLD` was stale, forcing `pkg/server` through 11 needless separate `go test` processes -- 34% pure overhead, directly responsible for release/test invocations running long enough to exceed this environment's own tool-call time limits. `docs/SUBSTRATE_TRACKING.md §1`'s wave-programme progress bars are now mechanically generated, not hand-typed, after Horacio noticed they rendered differently every time they were requested)
...
## [0.24.1] - 2026-08-03 (real multi-core confirmation, Horacio's own M1: G-17 -- the dxp/bal lock-order deadlock fix (0.24.0's own probe) -- now fully confirmed under true parallelism, both correctness and throughput. T-138 and T-136 closed)

- **G-17's multi-core confirmation, done.** `GOMAXPROCS=8 go test ./pkg/server/ -run TestObjAdversarial_EnsureSystemDxpDef_ConcurrentFirstUse -race -count=20 -v` on real Apple M1 silicon: **20/20 PASS, 15.962s total (~0.8s/run), zero 60s-class request timeouts, race detector silent throughout** -- against the pre-fix log's 9-of-10 failures at 60.5s each on the same hardware. Both halves of the guard's own pass condition (`docs/KNOWN_ISSUES.md`) are met: correctness (20/20, zero races) and throughput Horacio judges acceptable -- resolving 0.24.0's own explicitly-provisional "probe" framing, not just the correctness half. Bonus same-session evidence, unprompted: Horacio also re-ran G-13 (`bal` admission race, `-tags stress`, count=20) on the M1 -- PASS, 48.518s, confirming the fix introduced no regression to the adjacent admission-CAS path.
...
- **New dormant guard G-17** registered (`docs/KNOWN_ISSUES.md`), canonical invocation identical to the run that produced the diagnosing dump. Sandbox pass recorded as explicitly insufficient on its own, matching G-13/14/15/16's own precedent -- the guard's pass condition requires both zero 60s-class failures AND throughput numbers Horacio judges acceptable on real multi-core hardware, not correctness alone.
- **T-138 filed carrying the full diagnosis** (goroutine-by-goroutine site map across all five `dxp` adapters, both lock orders, the fix-direction options considered). **T-136 marked superseded**, its own hypothesis corrected in place rather than silently deleted. Neither closes this release -- both close together once Horacio's M1 rerun delivers the correctness-and-performance verdict his own decision named as the actual bar.
...
- **Dormant guards**: `G-14` (loc admission race harness) added this wave, sandbox-passed, multi-core confirmation owed to Horacio, not claimed as done — see `docs/KNOWN_ISSUES.md`. The pre-existing seven (G-01/02/03/04/05/11/13) remain stale for the same sandbox-hardware reason as every prior release; no guard-touching change to any of them this release.
